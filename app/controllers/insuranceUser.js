...
exports.createUser = async (req, res) => {
    ...
    try {
        ...
        const ccpPath = path.resolve(__dirname, '..', '..', 'test-network', 'organizations', 'peerOrganizations', 'insurance.jnj.com', 'connection-insurance.json');
        const ccp = JSON.parse(fs.readFileSync(ccpPath, 'utf8'));

        // Create a new CA client for interacting with the CA.
        const caURL = ccp.certificateAuthorities['ca.insurance.jnj.com'].url;
        const ca = new FabricCAServices(caURL);

        // Create a new file system based wallet for managing identities.
        const walletPath = path.join(process.cwd(), 'walletInsurance');
        ...
        const secret = await ca.register({
            affiliation: 'insurance.department1',
