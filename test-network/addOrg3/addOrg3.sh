...
function printHelp () {
  ...
  echo "      - 'up' - add insurance to the sample network. You need to bring up the test network and create a channel first."
  echo "      - 'down' - bring down the test network and insurance nodes"
...
function generateOrg3() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    set -x
    cryptogen generate --config=insurance-crypto.yaml --output="../organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
...
function generateOrg3Definition() {
  ...
  export FABRIC_CFG_PATH=$PWD
  set -x
  configtxgen -printOrg InsuranceMSP > ../organizations/peerOrganizations/insurance.jnj.com/insurance.json
  res=$?
  { set +x; } 2>/dev/null
  if [ $res -ne 0 ]; then
...
function addOrg3 () {
  ...
  if [ ! -d "../organizations/peerOrganizations/insurance.jnj.com" ]; then
    generateOrg3
  ...
  docker exec cli ./scripts/insurance-scripts/updateChannelConfig.sh $CHANNEL_NAME $CLI_DELAY $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
    fatalln "ERROR !!!! Unable to create config tx"
  fi

  infoln "Joining Org3 peers to network"
  docker exec cli ./scripts/insurance-scripts/joinChannel.sh $CHANNEL_NAME $CLI_DELAY $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
...
COMPOSE_FILE_COUCH_ORG3=docker/docker-compose-couch-insurance.yaml
# use this as the default docker-compose yaml definition
COMPOSE_FILE_ORG3=docker/docker-compose-insurance.yaml
# certificate authorities compose file
COMPOSE_FILE_CA_ORG3=docker/docker-compose-ca-insurance.yaml
# default image tag
IMAGETAG="latest"
...
if [ "$MODE" == "up" ]; then
  infoln "Adding insurance to channel '${CHANNEL_NAME}' with '${CLI_TIMEOUT}' seconds and CLI delay of '${CLI_DELAY}' seconds and using database '${DATABASE}'"
  echo
elif [ "$MODE" == "down" ]; then
  ...
  EXPMODE="Joining another channel"
else
  printHelp
  exit 1
fi
...
if [ "${MODE}" == "up" ]; then
  ...
  generateOrg3
  generateOrg3Definition
elif [ "${MODE}" == "join" ]; then ## join channel
  join
else
  printHelp
  exit 1
