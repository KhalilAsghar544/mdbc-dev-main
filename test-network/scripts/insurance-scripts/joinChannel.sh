...
# second step of the EYFN tutorial. It joins the insurance peers to the
...
set -x
peer channel fetch 0 $BLOCKFILE -o orderer.jnj.com:7050 --ordererTLSHostnameOverride orderer.jnj.com -c $CHANNEL_NAME --tls --cafile $ORDERER_CA >&log.txt
res=$?
{ set +x; } 2>/dev/null
cat log.txt
...
successln "Insurance peer successfully added to network"
